 ...
 * SCI.icon('name') → <svg> element; SCI.hydrateIcons(root) fills [data-ico]. */
...
SCI.icon = (name, cls) => {
  ...
  svg.innerHTML = SCI.icons[name] || SCI.icons.file;
  return svg;
};

SCI.hydrateIcons = (root) => {
  (root || document).querySelectorAll('[data-ico]').forEach(n => {
    if (!n.querySelector('svg')) n.append(SCI.icon(n.dataset.ico));
  });
};
